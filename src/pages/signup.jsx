...
import { getAuthToken } from "../api/utilities.jsx";
...
const Signup = () => {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      try {
        ...
        if (res.status === 200 && res.data.user) { navigate("/app"); return; }
      } catch {}
  ...
  return (
    <>
      ...
      <div style={{ minHeight:"100vh", display:"flex", background:"var(--bg)", fontFamily:"var(--font-body)" }}>
        ...
        <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"2rem 1.5rem" }}>
          <motion.div initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }}
            transition={{ duration:0.45, ease:[0.22,1,0.36,1] }} style={{ width:"100%", maxWidth:"420px" }}>
            ...
            <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
              ...
              <div>
                ...
                <div style={{ position:"relative" }}>
                  ...
                  <input className="input" type={showPassword ? "text" : "password"} autoComplete="new-password"
                    placeholder="Create a strong password" required value={password} onChange={e=>setPassword(e.target.value)}
              ...
              <p style={{ fontSize:"0.75rem", color:"var(--text3)", margin:"0.1rem 0" }}>
                By signing up you agree to our <span style={{ color:"var(--primary)", fontWeight:600 }}>Terms</span> and <span style={{ color:"var(--primary)", fontWeight:600 }}>Privacy Policy</span>.
              </p>
      ...
      <style>{`@media (min-width: 768px) { .signup-panel { display: flex !important; } }`}</style>
    </>
  );
};
